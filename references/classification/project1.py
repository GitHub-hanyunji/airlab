...
form_class2 = uic.loadUiType("untitled02.ui")[0]
...
class Image_classification(QTabWidget):
    def __init__(self):
        super().__init__()
        #self.initUI()
        uic.loadUi("untitled.ui", self)  # UI를 로드하여 self에 연결
...
# class WindowClass(QMainWindow):  # QTabWidget이 아닌 QMainWindow 사용
#     def __init__(self):
#         super().__init__()
#         self.setupUi(self)
#         # 탭 위젯 가져오기
#         self.classification_tab = self.findChild(QWidget, 'tab_1')
#         self.inference_tab = self.findChild(QWidget, 'tab')
        
#         if self.classification_tab is not None:
#             self.classification_ui = Image_classification()
#             layout = QVBoxLayout(self.classification_tab)
#             layout.addWidget(self.classification_ui)
#             self.classification_tab.setLayout(layout)
        
#         if self.inference_tab is not None:
#             self.inference_ui = Test()
#             layout = QVBoxLayout(self.inference_tab)
#             layout.addWidget(self.inference_ui)
#             self.inference_tab.setLayout(layout)
class T(QWidget,form_class2):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.pushButton.clicked.connect(self.A)
    def A(self):
        self.R.setText(f"select folder path :")
        
class WindowClass(QTabWidget, form_class):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        #self.tab_widget = self.findChild(QTabWidget, 'DeepLearning_Machine')  # 'tabWidget'은 UI 파일에 정의된 객체 이름
        
        # T 클래스 인스턴스 생성
        self.tab_t = T()
        
        # QTabWidget에 탭 추가
        self.addTab(self.tab_t, "Tab T")
